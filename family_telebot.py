...
def get_google_service(service_account_path, api='sheets'):
    """return connection to google api
    api='sheets'
    api='drive'
    """
    credentials = ServiceAccountCredentials.from_json_keyfile_name(service_account_path, ['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive'])
...
def get_phrases():
    ...
    service = get_google_service(SERVICE_ACCOUNT_PATH, api='sheets')
    result = service.spreadsheets().values().batchGet(spreadsheetId=SPREADSHEET_ID, ranges=PAGE_NAME).execute()
...
def get_phrase(phrases):
    ...
    range = PAGE_NAME + '!' + SHOW_DATETIME_COLUMN + str(txt_row_number)
    write_to_gs(data, range)

    return txt, txt_meaning

def get_img():    
    img_file_id = get_img_id(IMG_FILE_IDS_DATES_FILE_NAME)
    service_google_drive = get_google_service(SERVICE_ACCOUNT_PATH, api='drive')
    request = service_google_drive.files().get_media(fileId=img_file_id)
    filename = f'{PATH}img.jpg'
...
def get_mem(img_filename, txt, txt_meaning):
...
def send_mem_to_telegram(bot, telebot_chat_id, mem):
    buf = io.BytesIO()
    mem.save(buf, format='JPEG')
    byte_mem = buf.getvalue()
    bot.send_photo(telebot_chat_id, photo=byte_mem)
    return None

def write_img_file_ids_date_to_csv(img_file_ids_dates, img_file_ids_dates_name):
    with open(img_file_ids_dates_name, 'w') as fp:
            for item in img_file_ids_dates:
                fp.write("%s\n" % ','.join(item))
    return img_file_ids_dates_name
...
def get_img_file_ids_date_from_csv(img_file_ids_dates_name):
    """Get img_id and datetime last show
    """
    # if IMG_FILE_IDS_DATES_FILE_NAME is not exists then create it
    if not os.path.exists(img_file_ids_dates_name):
        img_file_ids_dates = get_img_file_ids_from_gdrive()
        write_img_file_ids_date_to_csv(img_file_ids_dates,img_file_ids_dates_name)

    img_file_ids_dates = []
    with open(img_file_ids_dates_name, 'r') as fp:
        for line in fp:
            x = line[:-1].split(',')
            img_file_ids_dates.append(x)
    return img_file_ids_dates
...
def get_item_to_show(items):
    ...
    # get random shown item
    perc_rnd = 0.5
    rnd_index = random.randint(0,math.ceil(len(items)*perc_rnd))
    res = items[rnd_index][0]

    not_shown = [i for i in items_sorted if not i[1]]
    # return item if there are new ones
    if not len(not_shown) == 0:   
        res = not_shown[0][0]
        
    return res

def get_img_id(img_file_ids_dates_name):
    img_file_ids_dates = get_img_file_ids_date_from_csv(img_file_ids_dates_name)
    ...
    write_img_file_ids_date_to_csv(img_file_ids_new_dates, img_file_ids_dates_name)
...
if __name__ == '__main__':  
    ...
    PATH = ""#"family_telebot/"
    SERVICE_ACCOUNT_PATH = f'{PATH}family-telebot-key.json'

    IMG_FILE_IDS_DATES_FILE_NAME = f'{PATH}img_show_dates.csv'

    # google spread sheet with phrases
    SPREADSHEET_ID = '1V7h9fE9OHSHQqdsAcv7nbP8aRlojy3M3781ghG6pq40'
    PAGE_NAME = 'phrases'
    SHOW_DATETIME_COLUMN = 'E'
    RESERVED_HEADER_ROWS = 1

    # google drive folder with photos 
    JULIA_PHOTOS_FOLDER_ID = '1-S0j2iKNVljjmL0Miu0_dFO9NYJba6Sn'

    # teleboot
    STEPANOVS_FAMILY_BOT_TOKEN = '5957062442:AAHM2qLBtZI-Ahguc4iFkevqzyrLbLdw69A'
    STEPANOVS_FAMILY_BOT_CHAT_ID = 1635188542
    FAMILY_CHAT_ID = -702408728
    ...
    while True:
        ...
        if cur_hour >= min_time and cur_hour <= max_time:
            ...
            img_filename = get_img()
            mem = get_mem(img_filename, txt, txt_meaning)
            send_mem_to_telegram(bot, telebot_chat_id, mem)
        
        time.sleep(random.randint(3*60*60,6*60*60))